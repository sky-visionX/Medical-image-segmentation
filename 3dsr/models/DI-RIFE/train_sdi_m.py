        ...
        for i, data in enumerate(train_data):
            ...
            if step % 1000 == 1 and local_rank == 0:
                ...
                for j in range(5):
                    ...
                    writer.add_image(str(j) + '/Meningioma_converted_preprocessed', imgs, step, dataformats='HWC')
...
def evaluate(model, val_data, nr_eval, local_rank, writer_val):
    ...
    for i, data in enumerate(val_data):
        ...
        for j in range(gt.shape[0]):
            ...
            # psnr = -10 * math.log10(torch.mean((merged_img[j] - gt[j]) * (merged_img[j] - gt[j])).cpu().Meningioma_converted_preprocessedraw)
        ...
        if i == 0 and local_rank == 0:
            for j in range(10):
                # imgs = np.concatenate((merged_img[j], pred[j], gt[j]), 1)[:, :, ::-1]
                imgs = np.concatenate((pred[j], gt[j]), 1)[:, :, ::-1]
                writer_val.add_image(str(j) + '/Meningioma_converted_preprocessed', imgs.copy(), nr_eval, dataformats='HWC')
...
if __name__ == "__main__":
    ...
    *****************************************************************
    ...
    parser.add_argument('--triplet', action='store_true', help='whether to use triplet Meningioma_converted_preprocessedraw')
