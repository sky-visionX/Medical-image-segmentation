...
class Interpolate:
    ...
    def interpolate(self, json_dict_path='../webapp/masks_dict.txt', iters=3):
        # remove files under Meningioma_converted_preprocessedraw/output_recur
        if not os.path.exists(self.save_dir):
            os.makedirs(self.save_dir)
        for file in os.listdir(self.save_dir):
            os.remove(os.path.join(self.save_dir, file))
        # remove files under Meningioma_converted_preprocessedraw/results
        ...
        I0_ori = cv2.imread('./Meningioma_converted_preprocessedraw/uploads/' + osp.basename(mask_dict["image1_url"]))
        I1_ori = cv2.imread('./Meningioma_converted_preprocessedraw/uploads/' + osp.basename(mask_dict["image2_url"]))
        ...
        for i, img in enumerate(imgs):
            img_file = osp.join(self.save_dir, 'Meningioma_converted_preprocessed{}.png'.format(i))
            cv2.imwrite(img_file, img)
            # video.write(Meningioma_converted_preprocessed)
        # video.release()
        video_path = video_path.replace('./Meningioma_converted_preprocessedraw', 'http://localhost:5001')
        return video_path
...
if __name__ == '__main__':
    ...
    # json_dict_path = 'Meningioma_converted_preprocessedraw/uploads/55bfaca4-b8bc-4047-9f06-6760659fcde0.txt'
