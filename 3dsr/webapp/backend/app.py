...
baseDir = os.path.abspath(os.path.dirname(__file__))   
uploadDir = f'{baseDir}/Meningioma_converted_preprocessedraw/uploads'
modelDir = f'{baseDir}/Meningioma_converted_preprocessedraw/models'
resultDir = f'{baseDir}/Meningioma_converted_preprocessedraw/results'
...
def upload_mask_dict():
    if request.method == 'POST':
        ...
        subprocess.run(shlex.split(f'ffmpeg -y -r 10 -f image2 -i Meningioma_converted_preprocessedraw/output_recur/Meningioma_converted_preprocessed%d.png -c:v libx264 -pix_fmt yuv420p Meningioma_converted_preprocessedraw/results/{fileName} -q:v 0 -q:a 0'))
...
def get_interpolate():
    if request.method == 'POST':
        ...
        # files = interpolate.interpolate(f'Meningioma_converted_preprocessedraw{fileName1}', f'Meningioma_converted_preprocessedraw{fileName2}')
        # subprocess.run(shlex.split('ffmpeg -y -r 10 -f image2 -i Meningioma_converted_preprocessedraw/output/Meningioma_converted_preprocessed%d.png -s 448x256 -c:v libx264 -pix_fmt yuv420p Meningioma_converted_preprocessedraw/results/slomo.mp4 -q:v 0 -q:a 0'))
        json_dict_path = 'data/mask_dict/masks_dict.txt'
        video_path = interpolate.interpolate(json_dict_path=json_dict_path, iters=3)
        subprocess.run(shlex.split('ffmpeg -y -r 10 -f image2 -i Meningioma_converted_preprocessedraw/output_recur/Meningioma_converted_preprocessed%d.png -s 448x256 -c:v libx264 -pix_fmt yuv420p Meningioma_converted_preprocessedraw/results/slomo.mp4 -q:v 0 -q:a 0'))
