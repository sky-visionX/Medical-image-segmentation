...
class U2NETP(nn.Module):
    ...
    def forward(self,x):
        ...
        if self.epoch is not None:
            self._visualize_u2net_output(hx1,  "u2net_features", f"stage1_output_epoch_{self.epoch}.png")
        hx = self.pool12(hx1)

        #stage 2
        hx2 = self.stage2(hx)
        if self.epoch is not None:
            self._visualize_u2net_output(hx2, "u2net_features", f"stage1_output_epoch_{self.epoch}.png")
        hx = self.pool23(hx2)

        #stage 3
        hx3 = self.stage3(hx)
        if self.epoch is not None:
            self._visualize_u2net_output(hx3,  "u2net_features", f"stage1_output_epoch_{self.epoch}.png")
        hx = self.pool34(hx3)

        #stage 4
        hx4 = self.stage4(hx)
        if self.epoch is not None:
            self._visualize_u2net_output(hx4,  "u2net_features", f"stage1_output_epoch_{self.epoch}.png")
        hx = self.pool45(hx4)

        #stage 5
        hx5 = self.stage5(hx)
        if self.epoch is not None:
            self._visualize_u2net_output(hx5, "u2net_features", f"stage1_output_epoch_{self.epoch}.png")
        hx = self.pool56(hx5)

        #stage 6
        hx6 = self.stage6(hx)
        if self.epoch is not None:
            self._visualize_u2net_output(hx6, "u2net_features", f"stage1_output_epoch_{self.epoch}.png")
    ...
    def _visualize_u2net_output(self, feature_map, epoch, folder_name, file_name):
        """
